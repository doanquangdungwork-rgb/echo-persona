...
async function getOwnedPersona(id: string) {
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const persona = await getOwnedPersona(id);
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Old visual DNA is now stale. The project must be re-interpreted before chat uses it again.
